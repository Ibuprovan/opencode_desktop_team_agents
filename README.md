...
这会复制以下文件到 `~/.config/opencode/`：

| 源文件 | 目标 |
|--------|------|
| `global/opencode.jsonc` | 全局 Agent 定义与模型配置 |
| `global/agents/*.md` | 14 个 Agent 指令文件（PMO、Architect、DBA 等） |
| `global/plugins/pmo-audit.mjs` | PMO 调度审计插件（自动记录 task 调用） |
...
项目级 `opencode.json` 仅包含 `build` 默认入口和三个团队命令，不包含 Agent 定义（已迁移至全局）。

### 3. 重启 opencode

修改 `opencode.json` 或 `global/agents/*.md` 后，必须完全重启 opencode，运行中的会话不会热加载这些配置。
...
| 基础设施层 | `memory` | 记忆管理器：维护项目长期记忆、持久化关键决策到文件 | `project-memory.md` |
...
```
.
├── opencode.json                    # 项目配置（build + 团队命令入口）
├── AGENTS.md                        # V2.2 完整工作流文档
├── deploy.ps1                       # Windows 部署脚本
├── deploy.sh                        # Linux/macOS 部署脚本
├── global/                          # 全局配置（部署到 ~/.config/opencode/）
│   ├── opencode.jsonc               #   Agent 定义与模型配置
│   ├── agents/                      #   14 个 Agent 指令文件
│   └── plugins/
│       └── pmo-audit.mjs            #   PMO 调度审计插件
├── docs/                          # 文档输出目录
│   ├── prd.md
│   ├── architecture.md
│   ├── api-spec.md
