...
6. **独立运行要求**
   - 若你是被 PMO 唤起，必须读取对应 `docs/tasks/TASK-XXX.md`，并在提交实现后把状态推进到 `REVIEWS` 候选
   - 若用户直接点名你修 bug 或实现功能，你也必须写明本次改动关联的任务文件或补建任务文件，不能只改代码不留交接信息
   - 不要假装 reviewer 或 qa-engineer 已经验证，验证必须由它们或用户显式触发
